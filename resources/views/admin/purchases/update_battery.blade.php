...
@section('content')
                                    ...
                                    <select id="battery_id" name="battery_id" class="form-select">
                                        ...
                                        @foreach ($batteries as $battery)
                                            <option value="{{ $battery->id }}"
                                                data-purchase-price="{{ $battery->purchase_price }}">
                                                {{ $battery->type }} | {{ $battery->brand->brand_name }} |
                                                {{ $battery->model_name }} | RS: {{ $battery->purchase_price }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <select id="payment_type" name="payment_type" class="form-select" required>
                                @foreach ($paymentTypes as $paymentType)
                                    <option value="{{ $paymentType }}"
                                        {{ $paymentType == $purchase->payment_type ? 'selected' : '' }}>
                                        {{ $paymentType }}
                                    </option>
                                @endforeach
                                ...
                                @foreach ($DBPaymentTypes as $paymentType)
                                    <option value="{{ $paymentType->id }}"
                                        {{ $paymentType->id == $purchase->payment_type ? 'selected' : '' }}>
                        ...
                        <div id="cheque_fields" style="display: none;">
                            <div class="mb-4">
                                ...
                                <input type="text" id="cheque_number" name="cheque_number" class="form-control"
                                    placeholder="Enter cheque number" value="{{ $purchase->cheque_number }}" />
                            </div>

                            <div class="mb-4">
                                <label for="cheque_date" class="form-label">Cheque Date</label>
                                <input type="date" id="cheque_date" name="cheque_date" class="form-control"
                                    placeholder="Enter cheque date" value="{{ $purchase->cheque_date }}" />
                            </div>
                        </div>
    ...
    <script>
        document.getElementById('battery_id').addEventListener('change', function() {
