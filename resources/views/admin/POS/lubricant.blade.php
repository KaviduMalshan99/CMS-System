...
@section('content')
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-3">
                                <a href="{{ route('quotations.index') }}"><button type="button"
                                        class="btn btn-outline-warning">Lubricant quotation</button></a>
                            </div>
            ...
            document.addEventListener("DOMContentLoaded", function() {
                ...
                document.getElementById('tax_option').addEventListener('change', function() {
                    if (this.value === 'include') {
                        ...
                        const customerId = document.getElementById("phonenumber").value;
                        ...
                        if (customerId) {
                            fetch(`/admin/pos/customers/${customerId}/tax-info`)
                                ...
                                .then(data => {
                                    ...
                                    document.getElementById('modal_customer_name').value = data
                                        .customer_name;
                                    document.getElementById('tax_holder_name').value = data
                                        .tax_holder_name || '';
                        ...
                        if (customerId) {
                            fetch(`/admin/pos/customers/${customerId}/tax-info`)
                                ...
                                .then(data => {
                                    ...
                                    // Update the warning display based on tax details
                                    if (!data.tax_holder_name || !data.tax_number) {
                                        document.getElementById('tax_warning').style.display =
                                            'block';
                                        document.getElementById('addTaxButton').disabled = true;
                                        ...
                                        hiddenInput.type = 'hidden';
                                        hiddenInput.name =
                                            'tax_id'; // Same name as the select field
                                        hiddenInput.value = '';
                                        // Append the hidden input inside the form
                                        taxSelect.parentNode.appendChild(hiddenInput);
                                    } else {
                ...
                document.getElementById('tax_id').addEventListener('change', function(e) {
                    ...
                    let customer = document.getElementById('phonenumber');
                    let selectedOption = customer.options[customer.selectedIndex];
                    let customerId = selectedOption.textContent.trim();
                ...
                function calculateTotals() {
                    ...
                    // Array to hold details of lubricants (id, quantity, price)
                    let lubricantDetails = [];
                    ...
                    document.querySelectorAll(".order-details-wrapper").forEach(orderItem => {
                        ...
                        // Collect details of each lubricant (ID, quantity)
                        const id = orderItem.querySelector(".battery-id").getAttribute("data-id");
                        lubricantDetails.push({
                            id: id,
                            quantity: quantity
                        });
                    });
                    ...
                    totalItemElement.querySelector(".item-number:nth-child(1) .f-w-500").textContent =
                        `${totalItems} (Items ${lubricantDetails.map(detail => `id = ${detail.id}, item = ${detail.quantity}`).join(", ")})`;
                    ...
                    // Collect all product IDs for all_id field
                    const orderIds = lubricantDetails.map(item => item.id);
                    document.getElementById("all_id").value = orderIds.join(", ");

                    // Collect all quantities for unit field, separated by commas
                    const quantities = lubricantDetails.map(item => item.quantity);
                    document.getElementById("unit").value = quantities.join(", ");
  ...
  <div class="touchspin-wrapper">
    ...
    <input class="input-touchspin item-quantity mx-1 border border-info" type="number" value="0" data-id="item1" style="width:50px;">
    <button class="increment-touchspin btn-touchspin"><i class="fa fa-plus text-gray"></i></button>
            ...
            `;
                ...
                function updateDisplay(input) {
                    ...
                    document.getElementById(`display-${itemId}`).textContent = value;
                }

                // Handle button clicks
                document.querySelectorAll('.btn-touchspin').forEach(button => {
                    button.addEventListener('click', function() {
                        const wrapper = this.closest('.touchspin-wrapper');
                        const input = wrapper.querySelector('.input-touchspin');
                        let value = parseInt(input.value) || 0;

                        if (this.classList.contains('increment-touchspin')) {
                            value++;
                        } else if (value > 0) {
                            value--;
                        }
