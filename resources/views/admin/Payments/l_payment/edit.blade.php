        ...
        <form action="{{ route('l_payment.update', $l_payment->id) }}" method="POST">
            ...
            <div class="form-group">
                ...
                <select name="product_type" id="product_type" class="form-control" required>
                    <option value="batteries" {{ $l_payment->product_type == 'batteries' ? 'selected' : '' }}>Batteries
                    </option>
                    <option value="lubricant" {{ $l_payment->product_type == 'lubricant' ? 'selected' : '' }}>Lubricant
                    </option>
            ...
            <div class="form-group">
                ...
                <input type="number" name="amount" id="amount" class="form-control"
                    value="{{ old('amount', $l_payment->amount) }}" required>
                @error('amount')
                    <div class="alert alert-danger mt-2">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" name="payment_method" id="payment_method" class="form-control"
                    value="{{ old('payment_method', $l_payment->payment_method) }}" required>
            ...
            <div class="form-group">
                ...
                <input type="text" name="status" id="status" class="form-control"
                    value="{{ old('status', $l_payment->status) }}">
                @error('status')
                    <div class="alert alert-danger mt-2">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="number" name="discount" id="discount" class="form-control"
                    value="{{ old('discount', $l_payment->discount) }}">
                @error('discount')
                    <div class="alert alert-danger mt-2">{{ $message }}</div>
                @enderror
            </div>
